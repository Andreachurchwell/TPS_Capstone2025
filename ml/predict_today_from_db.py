...
def predict_max_temp():
    try:
        ...
        # Step 2: Fallback to today's forecast
        if row.empty:
            print("[DEBUG] No forecast found for tomorrow. Trying today instead.")
            fallback_query = """
            SELECT min_temp, wind_speed
        ...
        # Step 3: Final fallback to latest available forecast
        if row.empty:
            print("[DEBUG] No forecast found for today/tomorrow. Using latest available row.")
            latest_query = """
            SELECT min_temp, wind_speed
            FROM forecast_data
            WHERE city = 'Selmer'
            ...
            LIMIT 1
            """
            row = pd.read_sql_query(latest_query, conn)
